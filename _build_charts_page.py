...
(function(){
  ...
  const yr=yS(2.1);
  SVG.appendChild(el('line',{x1:x0,y1:yr,x2:x1,y2:yr,stroke:'#dd6f3e','stroke-width':1.5,'stroke-dasharray':'6 5'}));
  SVG.appendChild(tx(x1,yr-7,'Replacement — 2.1 children','repl',{'text-anchor':'end'}));
  const groups=[{k:'a',cx:(x0+(x0+x1)/2)/2+40,lab:'1965'},{k:'b',cx:((x0+x1)/2+x1)/2-10,lab:'2025'}];
  const bw=(x1-x0)/2*0.74;
  groups.forEach(g=>{let below=0;TFR.forEach(r=>{const v=r[g.k],cx=g.cx+jit(r.iso)*bw/2,cy=yS(v),lo=v<2.1;if(lo)below++;SVG.appendChild(el('circle',{cx:cx.toFixed(1),cy:cy.toFixed(1),r:4.3,fill:lo?'#dd6f3e':'#1d9e75','fill-opacity':0.72,stroke:lo?'#993c1d':'#0f6e56','stroke-width':0.5,'stroke-opacity':0.5}));});SVG.appendChild(tx(g.cx,y0+30,g.lab,'big',{'text-anchor':'middle','font-size':'22'}));SVG.appendChild(tx(g.cx,y0+52,below+' of '+TFR.length+' below','bigl',{'text-anchor':'middle'}));});
...
(function(){
  ...
  const M=[[1972,'Germany',1],[1992,'Russia',1],[2005,'Japan',1],[2015,'Spain',1],[2022,'China',-1]];
  M.forEach(m=>{const yy=m[0],nm=m[1],dir=m[2],x=xS(yy),y=cS(cumAt(yy));SVG.appendChild(el('circle',{cx:x,cy:y,r:4,fill:'#0c1a33'}));SVG.appendChild(tx(x+(dir>0?8:-8),y+(dir>0?-6:-14),nm,'lab',{'text-anchor':dir>0?'start':'end'}));SVG.appendChild(tx(x+(dir>0?8:-8),y+(dir>0?8:0),String(yy),'bigl',{'text-anchor':dir>0?'start':'end','font-size':'11'}));});
